---
title: "Correlação de variáveis e Regressão Linear"
...
A correlação é uma fórmula estatística que mede o grau de dependência de duas variáveis que acontecem em par. 

A correlação varia entre -1 a 1. Valores perto de 0 sugerem uma fraca correlação, especialmente valores entre -0,2 e +0,2 indicam que grande parte da variação da resposta é devida a outras causas e deveremos procurar outras variáveis explicativas.

A fórmula da correlação (_corr_) é:

$$\rho_{XY} = corr =  \frac{ cov(X,Y)} {\sigma_X \sigma_Y}$$

ou

$$ \ corr =\frac{\sum_{n=1}^{n} (x_i-\bar x)(y_i-\bar y)}{\sqrt{[\sum_{i=1}^n(x_i-\bar x)^2][\sum_{i=1}^n(y_i-\bar y)^2]}} $$

---

##### Exemplo

Para o análise vamos usar a bem conhecida base de dados _cars_ do **R**, que contém 50 observações (linhas) com 2 variáveis (colunas).

```{r}
# Vamos ver as primeiras 10 linhas do arquivo
head(cars, 10)
...
O resultado indica que existe uma forte correlação positiva entre as duas variáveis "speed" e "dist".
...
A regressão linear é usada quando queremos estabelecer a relação entre o valor de uma variável _Y_, chamada de resposta, baseado em uma ou mais variáveis preditores ou explicativas, denominadas _Xs_. Ela é denominada de "linear" já que a relação entre as variáveis é uma função linear de primeiro grau de alguns parametros.

O objetivo é encontrar uma relação matematica linear através de uma fórmula, que relacione a(s) variáveis explicativas e a variável resposta, de forma que, quando os valores das variáveis explicativas são conhecidas, é possível estimar o valor da variável resposta.
...
Qualquer observação que exceda  uma vez e meia a amplitude inter-quartílica, ou seja ($1,5 * (Q3-Q1)$), é considerado um valor discrepante ou extremo (outlier), $Q3$ e $Q1$ são os 75 e 25 valores de percentís da variável considerada. Os valores de outliers ficam assim definidos como $x < Q1 - (1,5 * (Q3-Q1))$ e $x < Q3 + (1,5 * (Q3-Q1))$.

```{r reg02}
par(mfrow=c(1, 2))  # dividir o espaço gráfico em duas colunas

boxplot(cars$speed, 
        main = "Velocidade", 
        sub  = paste("Valores extremos: ", boxplot.stats(cars$speed)$out))

boxplot(cars$dist, 
        main = "Distância", 
        sub  = paste("Valores extremos: ", boxplot.stats(cars$dist)$out)) 
```
...
plot(density(cars$speed), 
     ...
     ylab = "Densidade", 
     sub  = paste("Assimetria:", round(e1071::skewness(cars$speed), 2)))  # density plot for 'speed'
polygon(density(cars$speed), col = "red")

plot(density(cars$dist), 
     main = "Gráfico de densidad \n para Distância", 
     ylab = "Densidade", 
     sub  = paste("Assimetria:", round(e1071::skewness(cars$dist), 2)))  # density plot for 'dist'
...
Antes devemos garantir que os resultados são estatísticamente significativos, para isso vamos ver algumas estatistísticas de qualidade.
...
##### O valor _p_: verificando a significancia estatística do modelo.
...
```{r reg10}
real_cal   <- data.frame(cbind(reais = teste_dados$dist, 
                               cal   = dist_calculada))
...
A metodologia consiste separar os dados em k-partes mutuamente exclusivas de amostras treino-teste. Isto é feito para cada "k" amostra aleatória e finalmente promediamos os "k" erros medios quadráticos. Podemos usar essa metodologia para comparar diferentes modelos lineares.
...
```{r reg12, warning=FALSE, message=FALSE}
library(DAAG)
par(mfrow = c(1, 1)) 
